...
export function generateSidebarItem(sidebarItems: DefaultTheme.SidebarItem[], path: string): DefaultTheme.SidebarItem[] {
...
export function getData(path: string, options?: Partial<Options>): DefaultTheme.SidebarItem {
  const src = readFileSync(path, "utf-8");
  const { data } = matter(src);
  const { title = this.getTitleFromContent(src) || basename(path), items } = data;
...
export function getTitleFromContent(content: string): string | undefined {
  return content
    .split("\n")
    .map((line) => line.trim())
