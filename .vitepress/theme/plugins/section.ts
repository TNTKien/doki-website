...
export const sections: SectionData[] = fg
  ...
  .map((file) => {
    ...
    const { title = getTitleFromContent(content) || basename(file), translator, logo, categories = [], dependencies = [], items, repository, description } = data;
...
export function prepareData(pageData: DocsPageData, siteConfig: SiteConfig): DocsPageData {
  ...
  pageData.title =
    !pageData.frontmatter.title && pageData.breadcrumbs.length
      ? pageData.breadcrumbs
          .map((item) => item.text)
          .reverse()
          .join(siteConfig.userConfig.themeConfig.titleSeparator)
      : pageData.title;

  if (section && !pageData.description && pageData.section.description) {
