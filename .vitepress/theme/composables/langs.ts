...
export function useLangs({ removeCurrent = true, correspondingLink = false } = {}) {
  ...
  const localeLinks = computed(() =>
    Object.entries(site.value.locales).flatMap(([key, value]) => {
      ...
      const link = normalizeLink(rootLink, theme.value.i18nRouting !== false && correspondingLink, page.value.relativePath.slice(currentLang.value.link.length - 1), !site.value.cleanUrls);

      if (link === "/" || Object.entries(site.value.locales).some(([key]) => link.replace(/^\//, "").replace(/\/$/, "") === key)) {
        return {
          text,
          link,
        };
